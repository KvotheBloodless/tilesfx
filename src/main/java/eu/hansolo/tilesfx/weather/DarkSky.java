...
public class DarkSky {
    ...
    public enum Unit {
        ...
        UK("uk", "\u00B0C", "mb", "\u0025", "mph"),
        UK2("uk2", "\u00B0C", "mb", "\u0025", "mph"), // same as SI except that nearest storm distance and visibility are in miles and wind
                                                      // speed is in mph
    ...
    public void setToday(final DataPoint TODAY) {
    ...
    public void setForecast(final List<DataPoint> FORECAST) {
    ...
    private void setDataPoint(final DataPoint DATA_POINT, final JSONObject JSON_OBJ, final TimeZone TIME_ZONE) {
        ...
        DATA_POINT.setPrecipIntensityMaxTime(
                epochStringToLocalDateTime(JSON_OBJ.getOrDefault("precipIntensityMaxTime", 0).toString(), TIME_ZONE));
        DATA_POINT.setPrecipProbability(Double.parseDouble(JSON_OBJ.getOrDefault("precipProbability", 0).toString()));
        DATA_POINT
                .setPrecipType(PrecipType.valueOf(JSON_OBJ.getOrDefault("precipType", "none").toString().toUpperCase().replace("-", "_")));
        ...
        DATA_POINT.setApparentTemperatureMinTime(
                epochStringToLocalDateTime(JSON_OBJ.getOrDefault("apparentTemperatureMinTime", 0).toString(), TIME_ZONE));
        ...
        DATA_POINT.setApparentTemperatureMaxTime(
                epochStringToLocalDateTime(JSON_OBJ.getOrDefault("apparentTemperatureMaxTime", 0).toString(), TIME_ZONE));
    ...
    private String createUrl(final double LATITUDE, final double LONGITUDE, final Unit UNIT, final Language LANGUAGE,
            final Exclude... EXCLUDES) {

        final StringBuilder PARAMETERS = new StringBuilder().append(String.format("%.4f", LATITUDE))
                .append(",")
                .append(String.format("%.4f", LONGITUDE))
                .append("?")
                .append("units=")
                .append(UNIT.value)
                .append("&")
