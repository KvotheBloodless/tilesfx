...
public class WeatherTileSkin extends TileSkin {
    ...
    protected void initGraphics() {
        ...
        this.forecast = tile.getForecast();
        this.forecastTempUnit = tile.getForecastTempUnit();
        ...
        unitText = new Text(tile.getForecastTempUnit());
    ...
    protected void resizeDynamicText() {
        ...
        if (unitText.isVisible()) {
            valueText.relocate(width - size * 0.075 - valueText.getLayoutBounds().getWidth() - unitText.getLayoutBounds().getWidth(),
                    size * 0.15);
        } else {
        ...
        if (sunriseText.getLayoutBounds().getWidth() > maxWidth) {
            Helper.adjustTextSize(sunriseText, maxWidth, fontSize);
        }

        sunsetText.setFont(Fonts.latoRegular(fontSize));
        if (sunsetText.getLayoutBounds().getWidth() > maxWidth) {
            Helper.adjustTextSize(sunsetText, maxWidth, fontSize);
        }
    }
    ...
    protected void redraw() {
        ...
        valueText.setText(String.format(Locale.US, "%.0f", forecast.getTemperatureMax()));
        unitText.setText(forecastTempUnit);
        weatherSymbol.setCondition(DarkSky.ConditionAndIcon.fromValue(forecast.getIcon()));
        text.setText(normalize(forecast.getSummary()));
        sunriseText.setText(TF
                .format(ZonedDateTime.ofInstant(forecast.getSunriseTime(), ZoneId.of(tile.getForecastTimezone()))));
        sunsetText.setText(TF
                .format(ZonedDateTime.ofInstant(forecast.getSunsetTime(), ZoneId.of(tile.getForecastTimezone()))));
        resizeDynamicText();
