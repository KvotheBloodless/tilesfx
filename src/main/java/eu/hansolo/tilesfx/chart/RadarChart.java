...
public class RadarChart extends Region {
    ...
    private void initGraphics() {
        ...
        // Add all nodes
        pane = new Pane(chartCanvas, overlayCanvas, unitText, minValueText, legend1Text, legend2Text, legend3Text, legend4Text,
                maxValueText);
    ...
    public ReadOnlyDoubleProperty minValueProperty() {
        ...
        if (null == minValue) {
            minValue = new DoublePropertyBase(_minValue) {
                ...
                @Override
                public void set(final double MIN_VALUE) {
    ...
    public int getNoOfSectors() {
        ...
        return null == noOfSectors ? _noOfSectors : noOfSectors.get();
    }
    ...
    public IntegerProperty noOfSectorsProperty() {
        ...
        if (null == noOfSectors) {
            noOfSectors = new IntegerPropertyBase(_noOfSectors) {
                ...
                @Override
                public void set(final int SECTORS) {
    ...
    public void addGradientStop(final Stop STOP) {
    ...
    public boolean isLegendVisible() {
        ...
        return null == legendVisible ? _legendVisible : legendVisible.get();
    }
    ...
    public void setData(final List<ChartData> DATA) {
        ...
        if (DATA.size() < MIN_NO_OF_SECTORS)
            throw new IllegalArgumentException("Not enough sectors (min. " + MIN_NO_OF_SECTORS + "needed)");
        if (DATA.size() > MAX_NO_OF_SECTORS)
            throw new IllegalArgumentException("Too many sectors (max. " + MAX_NO_OF_SECTORS + " sectors allowed)");
        DATA.forEach(d -> addData(d));
    }

    public void addData(final ChartData DATA) {

        if (data.size() > (getNoOfSectors() + 1))
    ...
    public Color getChartBackgroundColor() {
        ...
        return null == chartBackgroundColor ? _chartBackgroundColor : chartBackgroundColor.getValue();
    }
    ...
    public Color getChartForegroundColor() {
        ...
        return null == chartForegroundColor ? _chartForegroundColor : chartForegroundColor.getValue();
    }
    ...
    public Color getChartTextColor() {
        ...
        return null == chartTextColor ? _chartTextColor : chartTextColor.getValue();
    }
    ...
    public Color getGridColor() {
        ...
        return null == gridColor ? _gridColor : gridColor.getValue();
    }
    ...
    public Paint getChartFill() {
        ...
        return null == chartFill ? _chartFill : chartFill.getValue();
    }
    ...
    public Color getThresholdColor() {
        ...
        return null == thresholdColor ? _thresholdColor : thresholdColor.getValue();
    }
    ...
    public Mode getMode() {
        ...
        return null == mode ? _mode : mode.get();
    }
    ...
    private void redrawText() {
        ...
        unitText.setFont(Font.font("FontAwesome"));
        ...
        switch (getMode()) {
            case POLYGON:
                if (isSmoothing()) {
                    ...
                    double r3 =
                            (CENTER_Y - (CENTER_Y - OFFSET - ((data.get(NO_OF_SECTORS - 1).getValue() - MIN_VALUE) / DATA_RANGE) * RANGE));
                    ...
                    chartCtx.lineTo(interpolatedPoints[interpolatedPoints.length - 1].getX(),
                            interpolatedPoints[interpolatedPoints.length - 1].getY());
                    chartCtx.closePath();
    ...
    private void drawOverlay() {
        ...
        // draw star lines
        overlayCtx.save();
        for (int i = 0; i < NO_OF_SECTORS && false; i++) {
        ...
        if (isThresholdVisible()) {
            ...
            final double radius = ((getThreshold() - MIN_VALUE) / DATA_RANGE);
        ...
        for (int i = 0; i < NO_OF_SECTORS; i++) {
            ...
            for (final ChartData.Annotation annotation : data.get(i).getAnnotations()) {

                final Font font = annotation.getFont().orElse(Fonts.latoRegular(0.04 * size));

                final double offset = ((annotation.getValue() - MIN_VALUE) / DATA_RANGE) * RANGE + OFFSET + (font.getSize() / 1.5);
